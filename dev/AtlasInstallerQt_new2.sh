...
private:
    ...
    bool downloadWithFtp(const QString &url, const QString &outputPath) {
        ...
        // NOMBRE FIJO PARA REANUDACIÓN
        QString finalOutputPath = QDir::tempPath() + "/atlas_download.tmp";
        
        // Si hay archivo parcial en otra ubicación, moverlo al nombre fijo
        if (outputPath != finalOutputPath && QFile::exists(outputPath)) {
            qint64 size = QFileInfo(outputPath).size();
            if (size > 0) {
                emit logMessage(QString("🔄 Moviendo archivo parcial (%1) al nombre fijo para reanudación")
        ...
        if (QFile::exists(finalOutputPath)) {
            ...
            emit logMessage("🔄 La descarga se reanudará desde este punto");
        }
        
        QProcess wgetProcess;
        QStringList wgetArgs;
        
        // PARÁMETROS CORREGIDOS - eliminar conflicto entre --progress y -c
        wgetArgs << "--no-passive-ftp";
        wgetArgs << "-c";  // CRÍTICO: reanudar
        wgetArgs << "--progress=dot:giga";  // Usar formato más simple
        wgetArgs << "-O" << finalOutputPath;
        wgetArgs << "--tries=3";
        wgetArgs << "--timeout=60";
        wgetArgs << "--waitretry=10";
        
        // Eliminar barra de progreso detallada que causa problemas
        // wgetArgs << "--progress=bar:force:noscroll"; // ← ELIMINAR
        ...
        connect(&wgetProcess, &QProcess::readyReadStandardError, this, [this, &wgetProcess, finalOutputPath]() {
            ...
            if (!output.trimmed().isEmpty()) {
                ...
                while (percentMatches.hasNext()) {
                    ...
                    // Progreso simple para evitar conflictos
                ...
                if (output.contains("Resuming")) {
                    emit logMessage("🔄 Reanudando descarga previa...");
        ...
        // Copiar al outputPath original si es diferente
    ...
    bool downloadWithWgetResumable(const QString &url, const QString &outputPath) {
        // Siempre usar nombre fijo para reanudación
        QString finalOutputPath = QDir::tempPath() + "/atlas_download.tmp";
        
        // Verificar si ya tenemos progreso
        if (QFile::exists(finalOutputPath)) {
            qint64 existingSize = QFileInfo(finalOutputPath).size();
            if (existingSize > 0) {
                emit logMessage(QString("🔄 Archivo parcial encontrado: %1").arg(formatBytes(existingSize)));
            }
        }
        ...
        if (url.contains("localhost") || url.contains("127.0.0.1") || url.startsWith("file://")) {
            ...
            // ... resto del código local
            return runWgetProcess(wgetProcess, wgetArgs, "local");
        }
        
        // Para Google Drive
        emit logMessage("🌐 Usando método para Google Drive...");
        
        QString fileId = m_driveId;
        if (url.contains("id=")) {
        ...
        // Construir URL con token simple (funciona mejor para reanudación)
        QString downloadUrl = QString("https://drive.google.com/uc?id=%1&export=download&confirm=t")
                                .arg(fileId);
        
        emit logMessage("🔗 URL: " + downloadUrl);
        ...
        wgetArgs << "-O" << finalOutputPath;
        wgetArgs << "--tries=3";
        wgetArgs << "--timeout=60";  // Más tiempo para Google Drive
        wgetArgs << "--waitretry=10";
        wgetArgs << downloadUrl;
        
        return runWgetProcess(wgetProcess, wgetArgs, "Google Drive");
    }
...
void InstallerWindow::startInstallation()
{
    ...
    if (hasPartialDownload) {
        resumeText = QString(
            "<p><b>🔄 SE REANUDARÁ DESDE: %1 descargados</b></p><br>"
        ).arg(formatBytes(partialSize));
    }
    ...
    if (hasPartialDownload) {
        ...
        logText->append("[" + QDateTime::currentDateTime().toString("hh:mm:ss") + "] 📊 Progreso guardado: " + formatBytes(partialSize));
    } else {
