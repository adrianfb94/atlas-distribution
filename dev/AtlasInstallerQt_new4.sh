...
private:
    ...
    bool downloadWithFtp(const QString &url, const QString &outputPath) {
        ...
        QString finalOutputPath = QDir::tempPath() + "/atlas_download.tmp"; // <-- NOMBRE FIJO SIEMPRE
        ...
        if (QFile::exists(finalOutputPath)) {
            ...
            if (existingSize > 0) {
                ...
                emit progressUpdated(10, "Reanudando descarga previa...");
            }
        }
        
        // IGNORAR outputPath que viene como parámetro - usar solo finalOutputPath
        emit logMessage("⚠️  Ignorando ruta temporal proporcionada, usando ruta fija para reanudación");
        
        QProcess wgetProcess;
        QStringList wgetArgs;
        
        // PARÁMETROS CORREGIDOS - GARANTIZAR REANUDACIÓN
        wgetArgs << "--no-passive-ftp";
        wgetArgs << "-c";  // CRÍTICO: reanudar
        wgetArgs << "--progress=dot:giga";
        wgetArgs << "-O" << finalOutputPath; // <-- USAR SOLO ESTA RUTA
        wgetArgs << "--tries=3";
        wgetArgs << "--timeout=60";
        wgetArgs << "--waitretry=10";
        wgetArgs << "--retry-connrefused";
        wgetArgs << "--continue"; // OTRA FORMA DE ESPECIFICAR REANUDACIÓN
