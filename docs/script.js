...
(function detectAndFixEdge() {
  // Detectar Microsoft Edge (nuevo y viejo)
  ...
  if (isEdge) {
    console.log('🔧 Edge detectado - Aplicando correcciones...');
    
    // 1. Añadir clase al body
    document.body.classList.add('edge-browser');
    
    // 2. Aplicar fixes específicos
    applyEdgeFixes();
    
    // 3. Forzar reflow para aplicar CSS
  ...
  function applyEdgeFixes() {
    // Fix 1: Asegurar que los grids funcionen
    const grids = document.querySelectorAll('.platform-cards, .features-grid');
    grids.forEach(grid => {
      grid.style.display = 'grid';
      grid.style.gridGap = '2rem';
    });
    
    // Fix 2: Mejorar flexbox en stats
    ...
    // Fix 3: Remover backdrop-filter si existe
    ...
    // Fix 4: Asegurar que los botones sean clickeables
    const buttons = document.querySelectorAll('button, a.btn-primary, .cta-button');
    buttons.forEach(btn => {
      btn.style.position = 'relative';
      btn.style.zIndex = '10';
    });
    
    console.log('✅ Correcciones para Edge aplicadas');
  }
})();

// FINAL DE AL INICIO de tu script.js
